...
class Application:
    def __init__(self):
        ...
        self.console = ipw.Combobox(options = [
             '2DSXL',
             '3DS',
             '3DS XL',
             'DSi',
        ...
        self.retour = ipw.ToggleButtons(options = ["Sans retour", "Avec retour"], layout=ipw.Layout(width='50%', height='60px'))
        self.etat = ipw.ToggleButtons(description = "Etat de la console", options = ["Occasion", "Neuf", "HS / Pour pièces détachées", "Reconditionné"], layout=ipw.Layout(height='80px'))
        ...
        self.memoire_ps3 = ipw.Select(
            options=['Ne sait pas', '12', '40', '60', '80', '120', '150', '160', '250', '320', '500'],
            value='Ne sait pas',
            # rows=10,
            description='Memoire en giga:',
            disabled=False
    ...
    def affichage(self):
        sortie = ipw.interactive_output(
            self.construction, {"valeur": self.console}
        )
        widgets = ipw.VBox([
            ipw.HBox([self.console, ipw.VBox([self.cable, self.manette]), ipw.VBox([self.edition, self.nbjeu])]),
            self.retour, 
            self.deli_price,
            self.etat])
    ...
    def _sur_clique(self, b):
        variables = [0, self.deli_price.value, self.transforme_etat(), self.transforme_retour(), self.transforme_memoire(), self.console.value, self.transforme_manette(), self.transforme_cable(), self.nbjeu.value, self.transforme_edition()]
